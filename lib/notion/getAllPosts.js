...
function shouldTryOfficialApi () {
  return (
    BLOG.notionApiToken ||
    BLOG.notionDataSourceId ||
    BLOG.notionAccessToken?.startsWith('secret_') ||
    BLOG.notionAccessToken?.startsWith('ntn_')
  )
}

async function retrieveDefaultDataSourceId (token) {
  if (!BLOG.notionPageId) return null

  const response = await fetch(
    `https://api.notion.com/v1/databases/${BLOG.notionPageId}`,
    {
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Notion database retrieve failed: ${response.status} ${text}`)
  }
...
async function queryOfficialNotionDataSource () {
  if (!shouldTryOfficialApi()) return null

  const token = BLOG.notionApiToken || BLOG.notionAccessToken
  const dataSourceId =
    BLOG.notionDataSourceId || await retrieveDefaultDataSourceId(token)
